...
class Live(Base):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(128), unique=True, index=True, nullable=False)
    liveuser_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='CASCADE'))
    liveuser = db.relationship('User', backref=db.backref('live', cascade='all, delete-orphan'))
