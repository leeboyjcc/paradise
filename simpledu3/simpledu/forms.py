...
class UserForm(FlaskForm):
    username = StringField('用户名', validators=[DataRequired(), Length(3, 24, message='用户名长度要在3到24个字符之间')])
    email = StringField('邮箱', validators=[DataRequired(), Email(message='请输入合法的email地址')])
    password = PasswordField('密码', validators=[DataRequired(), Length(6, 24, message='密码长度要在6到24个字符之间')])
    role = IntegerField('角色', validators=[Optional()], filters=[lambda x: x or None])
    job = StringField('职业', validators=[Optional(), Length(max=128)])
    submit = SubmitField('提交')
