...
def count_top10():
    session = Session()
    rows = session.query(
        JobModel.city, func.count(JobModel.city).label('count')
...
def experience_stat():
    ...
    rows = session.query(
        func.concat(
            JobModel.experience_lower, '-', JobModel.experience_upper
            ).label('experience'), func.count('experience').label('count')
...
def salary_by_city_and_education():
    session = Session()
    rows = session.query(
        JobModel.city, JobModel.education, func.avg(
            (JobModel.salary_lower + JobModel.salary_upper) / 2
            ).cast(Float).label('salary')
        ).filter(
        and_(JobModel.salary_lower > 0, JobModel.salary_upper > 0)
